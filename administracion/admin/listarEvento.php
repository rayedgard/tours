...
function direc(d,u)
{
var r=confirm("Esta seguro que desea eliminar este EVENTO: "+u+" !!");
if (r==true)
  {
...
$codigoEventos=$_GET['cod'];
$eliminado=$_GET['e'];
// consulta para onçbtener el registro para eliminar la imagen de la noticia
$consultaEvento = mysql_query("select imagen from eventos where id='$codigoEventos'",$link);
$eventos= mysql_fetch_array($consultaEvento);

if($eliminado=='2')
	{
//eliminando de la bd 

mysql_query("DELETE FROM eventos WHERE id='$codigoEventos' ",$link);
	unlink("../imagenes/eventos/".$eventos[0]);
$my_error = mysql_error($link);
	}
	if($eliminado=='1')
	{
mysql_query("UPDATE eventos SET  eliminar='1' WHERE id='$codigoEventos'",$link);
$my_error = mysql_error($link);
	}
	
	if($eliminado=='0')
	{
mysql_query("UPDATE eventos SET  eliminar='0' WHERE id='$codigoEventos'",$link);
$my_error = mysql_error($link);
	}
...
$consulta2 ="SELECT id,nombre,fecha,eliminar,idioma FROM eventos ORDER BY fecha DESC"; 
...
while($row2 = mysql_fetch_array($resultado2))
		...
		array_push($eliminar,$row2[3]);
		array_push($idioma,$row2[4]);
		}
...
<table width="100%" border="0">
  <tr border="1" bgcolor="#996600">
    ...
    <td width=""><font color="#ffffff">Nombre Evento</font></td>
    <td width=""><font color="#ffffff">Fecha Evento</font></td>
...
$cont=1;
 ...
 for($j=0;$j<count($nombreEvento);$j++)
{
